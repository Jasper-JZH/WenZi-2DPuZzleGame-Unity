...
public class Pen : MonoBehaviour
{
    ...
    [Header("Gesture")]
    [SerializeField] private List<Gesture> targetGestureList = new List<Gesture>();
    private void Awake()
    {
    ...
    private void Start()
    {
        ...
        GestureIO.ReadGestureObjectFromJson(out tempGestureObejct, Application.streamingAssetsPath + "/test Gesture 1.text");
        targetGestureList.Add(new Gesture(tempGestureObejct));
    }
    ...
    private void Update()
    {
        ...
        if(Input.GetKeyUp(KeyCode.C))   //结束绘制
        {
            ...
            if (strokeCount > 0)
            {
                ...
                //调用笔迹匹配方法
                //......
               Debug.Log($"匹配程度（平均距离为）{GestureIO.GestureMatch(newGesture, targetGestureList[0], Gesture.SAMPLE_POINT_NUM)}");
            }
        }
